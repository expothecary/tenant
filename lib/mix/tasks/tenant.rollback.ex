defmodule Mix.Tasks.Tenant.Rollback do
  ...
  config under tenant configuration. For example, to use "priv/repo/my_migrations":

      config :tenant, migrations_path: "my_migrations"
  ...
  @impl true
  def run(args, migrator \\ &Tenant.run_tenant_migrations/2) do
    migrator.(args, :down)
  end
end
